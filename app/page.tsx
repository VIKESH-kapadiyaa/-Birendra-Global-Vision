...
const GatewayCard = ({ title, subtitle, enterLabel, icon: Icon, features, themeColor, onHover, isActive, href, onClick, image }: any) => {
    ...
    const CardContent = (
        ...
        >
            ...
            <div className="relative h-full w-full bg-black/40 backdrop-blur-md border border-white/10 rounded-2xl sm:rounded-[2rem] md:rounded-[3rem] p-4 sm:p-6 md:p-12 flex flex-col overflow-hidden">
                ...
                {image && (
                    <div className="absolute inset-0 w-full h-full pointer-events-none z-0">
                        <img
                            src={image}
                            alt={title}
                            className="w-full h-full object-cover transform scale-110 group-hover:scale-100 transition-transform duration-1000 ease-out opacity-20 group-hover:opacity-50"
                        />
...
export default function App() {
    ...
    const PROGRAMS = {
        genius: [
            { title: t.occultSciencesTitle, level: t.occultSciencesLevel, duration: t.occultSciencesDuration, desc: t.occultSciencesDesc, image: "/programs/bg_occult.png", color: "purple" },
            { title: t.lifeCoachingTitle, level: t.lifeCoachingLevel, duration: t.lifeCoachingDuration, desc: t.lifeCoachingDesc, image: "/programs/bg_coaching.png", color: "blue" },
            { title: t.mentoringTitle, level: t.mentoringLevel, duration: t.mentoringDuration, desc: t.mentoringDesc, image: "/programs/bg_mentoring.png", color: "purple" },
        ],
        life: [
            { title: t.academicSupportTitle, level: t.academicSupportLevel, duration: t.academicSupportDuration, desc: t.academicSupportDesc, image: "/programs/bg_academic.png", color: "blue" },
            { title: t.skillDevTitle, level: t.skillDevLevel, duration: t.skillDevDuration, desc: t.skillDevDesc, image: "/programs/bg_skills.png", color: "purple" },
            { title: t.holisticLearningTitle, level: t.holisticLearningLevel, duration: t.holisticLearningDuration, desc: t.holisticLearningDesc, image: "/programs/bg_holistic.png", color: "blue" },
        ],
    };
        ...
        <div suppressHydrationWarning className="relative min-h-[400vh] w-full bg-[#030303] overflow-x-hidden font-sans selection:bg-white/20 text-white">
                            ...
                            <div className="lg:col-span-7 order-1 lg:order-2 space-y-8 text-center lg:text-left">
                                ...
                                <div className="space-y-4 md:space-y-6 text-sm md:text-lg text-white/50 leading-relaxed font-light">
                                    ...
                                    <p className="hidden sm:block">
                                        From her tenure at prestigious institutions, to her leadership roles as Head of School and Academic Manager, she brings a wealth of experience to every mentorship session.
                                    </p>
                                </div>
            ...
            <section id="features" className="relative z-10 py-12 sm:py-16 md:py-32 px-3 sm:px-4 md:px-6 bg-[#050505] border-y border-white/5">
                <div className="container mx-auto max-w-7xl">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3 md:gap-6">
                        {FEATURES.map((feat, i) => (
                            <FeatureCard key={i} feat={feat} index={i} onClick={() => setSelectedFeature(feat)} />
                        ))}
                    </div>
                </div>
            </section>
            ...
            <section id="programs" className="relative z-10 py-12 sm:py-16 md:py-48 px-3 sm:px-4 md:px-6 bg-[#050505] border-y border-white/5 overflow-hidden">
                ...
                <div className="container mx-auto max-w-7xl">
                    ...
                    >
                        <SectionHeader subtitle={t.programsSubtitle || "PROGRAMS"} title={t.programsTitle || "CHOOSE YOUR PATH"} align="center" />
                    </motion.div>

                    {/* The Accordion Container */}
                    <div className="flex flex-col lg:flex-row gap-2 md:gap-4 h-[900px] sm:h-[1200px] lg:h-[600px] w-full relative z-10 overflow-hidden">
                        {PROGRAMS.genius.concat(PROGRAMS.life).map((item, i) => (
                            <ProgramCard key={i} item={item} index={i} color={item.color as "purple" | "blue"} image={item.image} />
                        ))}
                    </div>
                </div>
            </section>

            {/* GROW OUR FACULTY SECTION */}
            <section className="relative z-10 py-16 sm:py-20 md:py-40 px-3 sm:px-4 md:px-6 bg-[#020202] border-y border-white/5 overflow-hidden">
                {/* Radiant Glow */}
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[300px] md:w-[800px] h-[300px] md:h-[800px] bg-amber-500/10 rounded-full blur-[100px] md:blur-[200px] pointer-events-none" />
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[200px] md:w-[500px] h-[200px] md:h-[500px] bg-purple-600/10 rounded-full blur-[80px] md:blur-[150px] pointer-events-none mix-blend-screen" />
                ...
                <div className="container mx-auto max-w-5xl relative z-10 text-center">
                    ...
                    >
                        <span className="inline-block px-3 py-1 rounded-full bg-amber-500/10 border border-amber-500/20 text-amber-500 text-[10px] md:text-xs font-bold uppercase tracking-widest mb-6">
                            Global Faculty Network
                        </span>
                        <h2 className="text-3xl sm:text-4xl md:text-6xl lg:text-7xl font-black text-white leading-[1.1] tracking-tight mb-6">
                            SHAPE THE <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-400 via-yellow-200 to-amber-600">FUTURE.</span>
                        </h2>
                        <p className="text-white/60 text-sm md:text-xl font-medium max-w-2xl mx-auto leading-relaxed mb-10 md:mb-16">
                            Join Birendra Global Vision as a mentor. We are looking for passionate educators capable of breaking boundaries and igniting minds across the globe.
                        </p>
                    </motion.div>

                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 mb-12 md:mb-20 max-w-6xl mx-auto">
                        {[
                            { title: "Global Reach", desc: "Teach students worldwide from the comfort of your home.", icon: <Globe size={24} />, image: "/faculty/globe.png" },
                            { title: "Elite Community", desc: "Join a network of top-tier, hand-selected global educators.", icon: <Users size={24} />, image: "/faculty/mentor.png" },
                            { title: "Flexible Teaching", desc: "Set your own schedule and shape your unique curriculum.", icon: <Sparkles size={24} />, image: "/faculty/flex.png" },
                        ].map((stat, i) => (
                            <motion.div
                                key={i}
                                ...
                                className="relative group p-6 md:p-8 rounded-[2rem] md:rounded-[2.5rem] bg-[#0a0a0a] border border-white/10 hover:border-amber-500/30 transition-all duration-500 h-[350px] md:h-[450px] overflow-hidden flex flex-col items-center justify-end text-center shadow-xl hover:shadow-[0_20px_60px_-15px_rgba(245,158,11,0.2)]"
                            >
                                ...
                                <div className="absolute inset-0 w-full h-full pointer-events-none">
                                    <img
                                        src={stat.image}
                                        alt={stat.title}
                                        className="absolute inset-0 w-full h-full object-cover transform scale-[1.05] group-hover:scale-110 transition-transform duration-1000 ease-out opacity-30 group-hover:opacity-60"
                                    />
                                    {/* Heavy bottom vignette to make text pop */}
                                    <div className="absolute inset-0 bg-gradient-to-t from-[#020202] via-[#020202]/80 to-transparent" />
                                </div>

                                <div className="relative z-10 flex flex-col items-center transform group-hover:-translate-y-4 transition-transform duration-500">
                                    <div className="w-16 h-16 md:w-20 md:h-20 rounded-full bg-amber-500/10 backdrop-blur-md border border-amber-500/20 flex items-center justify-center text-amber-500 mb-6 group-hover:scale-110 group-hover:bg-amber-500/30 transition-all duration-500 shadow-[0_0_30px_rgba(245,158,11,0.2)]">
                                        <div className="scale-110 md:scale-125 group-hover:rotate-12 transition-transform duration-500">{stat.icon}</div>
                                    </div>
                                    <h4 className="text-2xl md:text-3xl font-black text-white mb-3 tracking-tight drop-shadow-lg">{stat.title}</h4>
                                    <p className="text-white/60 text-sm md:text-base font-medium max-w-[250px] leading-relaxed drop-shadow group-hover:text-white/90 transition-colors duration-500">{stat.desc}</p>
                                </div>
                            </motion.div>
                        ))}
                    </div>
                    ...
                    >
                        <Link href="/apply-teacher" className="inline-flex items-center gap-3 px-8 py-4 rounded-full bg-gradient-to-r from-amber-600 to-yellow-500 text-black font-bold uppercase tracking-wider text-sm hover:scale-105 hover:shadow-[0_0_40px_-10px_rgba(245,158,11,0.5)] transition-all duration-300">
                            Apply As An Educator <ArrowRight size={18} />
                        </Link>
                    </motion.div>
                </div>
            </section>
...
function ProgramCard({ item, index, color, image, onClick }: { item: any, index: number, color: "purple" | "blue", image: string, onClick?: () => void }) {
    const bgGlowColor = color === "purple" ? "bg-purple-500" : "bg-blue-500";
    ...
    return (
        <motion.div
            ...
            onClick={onClick}
            className={`group flex-1 hover:flex-[3_3_0%] lg:hover:flex-[4_4_0%] relative rounded-[1.5rem] md:rounded-[2.5rem] bg-[#0a0a0a] border border-white/10 overflow-hidden transition-all duration-700 ease-[cubic-bezier(0.25,1,0.5,1)] shadow-xl shadow-black cursor-pointer bg-cover bg-center`}
        >
            {/* Background Image */}
            <div className="absolute inset-0 w-full h-full pointer-events-none overflow-hidden rounded-[1.5rem] md:rounded-[2.5rem]">
                <img
                    src={image}
                    alt={item.title}
                    className="absolute inset-0 w-full h-full object-cover transform scale-[1.3] group-hover:scale-100 transition-transform duration-1000 ease-out opacity-40 group-hover:opacity-90"
                />
                <div className="absolute inset-0 bg-gradient-to-t from-black via-black/50 to-transparent opacity-90 group-hover:opacity-60 transition-opacity duration-700" />
                <div className={`absolute inset-0 opacity-0 group-hover:opacity-20 transition-opacity duration-700 blur-2xl ${bgGlowColor}`} />
            </div>

            {/* Collapsed State Title (Desktop Only) */}
            <div className="hidden lg:flex absolute inset-0 items-center justify-center opacity-100 group-hover:opacity-0 transition-opacity duration-500 pointer-events-none whitespace-nowrap">
                <h3 className="text-xl xl:text-3xl font-black text-white uppercase tracking-widest transform -rotate-90 drop-shadow-xl opacity-60">
                    {item.title}
                </h3>
            </div>

            {/* Active Content - Bottom left align */}
            <div className="absolute bottom-0 left-0 right-0 p-4 md:p-8 flex flex-col justify-end">
                {/* Mobile / Tablet default title + Desktop expanded title */}
                <div className="transition-all duration-700 transform translate-y-4 group-hover:translate-y-0 opacity-100 lg:opacity-0 lg:group-hover:opacity-100">
                    <div className="flex items-center gap-2 mb-2 lg:mb-3">
                        <span className="px-2 lg:px-3 py-1 bg-white/10 backdrop-blur-md rounded-full text-[8px] lg:text-xs font-bold uppercase tracking-widest border border-white/20 text-white shadow-sm truncate">
                            {item.level}
                        </span>
                        <span className="flex items-center gap-1.5 px-2 lg:px-3 py-1 bg-white/10 backdrop-blur-md rounded-full text-[8px] lg:text-xs font-bold uppercase tracking-widest border border-white/20 text-white/80 shadow-sm truncate hidden sm:flex">
                            <Clock size={12} className="min-w-fit" /> {item.duration}
                        </span>
                    </div>

                    <h3 className="text-lg sm:text-2xl lg:text-5xl font-black text-white leading-[1.1] tracking-tight mb-0 lg:mb-3 drop-shadow-lg truncate">
                        {item.title}
                    </h3>

                    {/* Expandable Text Body */}
                    <div className="grid grid-rows-[0fr] group-hover:grid-rows-[1fr] transition-[grid-template-rows] duration-700">
                        <div className="overflow-hidden">
                            <p className="text-white/70 text-xs lg:text-base leading-relaxed line-clamp-2 lg:line-clamp-3 mt-2 mb-4 font-medium drop-shadow-md">
                                {item.desc}
                            </p>

                            {/* CTA Link */}
                            <div className="flex items-center gap-2 text-[10px] lg:text-sm font-bold uppercase tracking-wider text-white pb-2 lg:pb-0">
                                <span>Explore Program</span>
                                <div className="w-6 h-6 lg:w-8 lg:h-8 rounded-full bg-white/20 backdrop-blur border border-white/30 flex items-center justify-center text-white shrink-0">
            ...
            <div className="absolute inset-0 rounded-[1.5rem] md:rounded-[2.5rem] border-[1px] border-white/10 pointer-events-none group-hover:border-white/40 transition-colors duration-700" />
        </motion.div>
    );
}

// --- Feature Card Component ---
function FeatureCard({ feat, index, onClick }: { feat: any, index: number, onClick: () => void }) {
    ...
    return (
        <motion.div
            layoutId={`feature-card-main-${index}`}
            onClick={onClick}
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true, margin: "-50px" }}
            ...
            className="group relative p-4 md:p-8 h-full min-h-[160px] md:min-h-[280px] rounded-2xl md:rounded-[2.5rem] bg-[#080808] border border-white/10 hover:border-purple-500/30 cursor-pointer overflow-hidden transition-all duration-500 shadow-xl flex flex-col hover:shadow-[0_20px_40px_-10px_rgba(168,85,247,0.15)]"
        >
            {/* Spotlight Interactive Hover Effect */}
            <motion.div
                className="pointer-events-none absolute -inset-px rounded-2xl md:rounded-[2.5rem] opacity-0 transition duration-300 group-hover:opacity-100 mix-blend-overlay"
                style={{
            ...
            {/* Bottom Glow */}
            <div className="absolute inset-x-0 bottom-0 h-[2px] bg-gradient-to-r from-transparent via-purple-500 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-700 blur-[2px]" />

            <div className="relative z-10 flex flex-col h-full">
                {/* Icon glowing block */}
                <div className="w-10 h-10 md:w-16 md:h-16 rounded-xl md:rounded-2xl bg-white/5 flex items-center justify-center text-white/40 mb-3 md:mb-6 group-hover:text-purple-400 group-hover:bg-purple-500/10 group-hover:scale-110 group-hover:-rotate-6 transition-all duration-500 shadow-sm">
                    <div className="scale-75 md:scale-125">{feat.icon}</div>
                </div>

                <h4 className="text-sm md:text-xl font-black mb-2 tracking-tight text-white leading-tight group-hover:text-white transition-colors duration-500 drop-shadow-sm">
                    {feat.title}
                </h4>

                <p className="text-white/40 text-[10px] md:text-sm leading-relaxed flex-grow line-clamp-3 md:line-clamp-4 font-medium group-hover:text-white/80 transition-colors duration-500">
                    {feat.desc}
                </p>

                {/* Discover Layer */}
                <div className="mt-4 flex flex-col sm:flex-row items-start sm:items-center gap-2 text-[10px] sm:text-xs font-bold uppercase tracking-wider text-purple-400 opacity-0 transform translate-y-4 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-500">
                    <span>Discover More</span>
                    <div className="w-6 h-6 md:w-8 md:h-8 rounded-full bg-purple-500/10 flex items-center justify-center group-hover:bg-purple-500 group-hover:text-white transition-colors duration-500 ml-1">
                        <ArrowRight size={14} className="scale-75 md:scale-100" />
                    </div>
                </div>
            </div>
        </motion.div>
